  ...
  const handleImageSubmit = () => {
    ...
    if (
      ...
      for (let i = 0; i < images.length; i++) {
        ...
        console.log(`Preparing to upload image ${i + 1}:`, {
          name: image.name,
        ...
        promises.push(
          storeImage(image).catch((err) => {
            console.error(`Image ${i + 1} upload failed:`, err);
            throw err; // Let Promise.all catch this
          })
        );
      }
      ...
      Promise.all(promises)
        ...
        .catch((err) => {
          ...
          setImageUploadError("Image upload failed (2mb max per image or permission denied)");
          setUploading(false);
        });
    } else {
      ...
      console.warn(`Upload aborted: ${reason}`);
