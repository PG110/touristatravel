...
const Home = () => {
  ...
  const getTopPackages = useCallback(async () => {
    try {
      ...
      const res = await fetch(
        "https://touristatravel-qujg9xexy-pg110s-projects.vercel.app/api/package/get-packages?sort=packageRating&limit=8"
      );
  ...
  const getLatestPackages = useCallback(async () => {
    try {
      setLoading(true);
      const res = await fetch(
        "https://touristatravel-qujg9xexy-pg110s-projects.vercel.app/api/package/get-packages?sort=createdAt&limit=8"
      );
      const data = await res.json();
      if (data?.success) {
  ...
  const getOfferPackages = useCallback(async () => {
    try {
      setLoading(true);
      const res = await fetch(
        "https://touristatravel-qujg9xexy-pg110s-projects.vercel.app/api/package/get-packages?sort=createdAt&offer=true&limit=6"
      );
      const data = await res.json();
      if (data?.success) {
